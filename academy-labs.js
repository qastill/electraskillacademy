...
(() => {
  ...
  /* Lab yang sudah dikerjakan sampai tuntas (panduan dicentang + 2 soal
     bukti paham benar) ditandai di kartunya. Sumber: localStorage yang
     ditulis /data/lab-panduan.js — dibaca langsung agar tidak bergantung
     urutan muat skrip. */
  function labSelesai(id) {
    try { const s = JSON.parse(localStorage.getItem('esa_labs_panduan') || '{}'); return !!(s[id] && s[id].selesai); }
  ...
  function kartu(lab) {
    ...
    return `
      <button type="button" class="prak-card" data-lab="${esc(lab.id)}" onclick="${j.buka(lab.id)}">
        ...
        <span class="prak-go">${labSelesai(lab.id) ? 'Buka lagi →' : 'Buka lab →'}</span>
        ${labSelesai(lab.id) ? '<span class="prak-done">✓ Selesai</span>' : ''}
      </button>`;
  }
